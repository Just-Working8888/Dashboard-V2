import datetime
import os
import schedule
import time

# Замените эти значения на свои данные
github_token = "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"
repo_url_with_token = f"https://Just-Working8888:{github_token}@github.com/Just-Working8888/Dashboard-V2.git"

def job():
    file_path = '/Users/bekbolotzairbekov/Desktop/GitHub/script.py'

    # Добавление новой строки с текущей датой
    new_line = f"# New line for {datetime.datetime.now()}\n"
    with open(file_path, 'a') as file:
        file.write(new_line)

    # Изменить URL репозитория на версию с токеном (только первый раз)
    os.system(f'git -C /Users/bekbolotzairbekov/Desktop/GitHub remote set-url origin {repo_url_with_token}')

    # Команды Git для добавления, коммита и пуша изменений
    os.system(f'git -C /Users/bekbolotzairbekov/Desktop/GitHub add {file_path}')
    os.system(f'git -C /Users/bekbolotzairbekov/Desktop/GitHub commit -m "Add new line for {datetime.datetime.now().date()}"')
    os.system('git -C /Users/bekbolotzairbekov/Desktop/GitHub push')

# Запланировать выполнение функции job каждую минуту
schedule.every(5).seconds.do(job)

while True:
    schedule.run_pending()
    time.sleep(1)